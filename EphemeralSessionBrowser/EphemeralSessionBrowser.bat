...
function Get-InstallPaths() {
  ...
  return [PSCustomObject]@{
    ...
    ProgramIcon       = Join-Path -Path $installDirectory -ChildPath "EphemeralSessionBrowser.ico"
...
"@
  ...
  Set-ItemProperty -Path $uninstallKey -Name "DisplayVersion" -Value $ESPVersion
  ...
  # create shortcuts
  if ($DesktopShortcut) {
    New-Shortcut -Path $paths.DesktopShortcut -Target $paths.LoaderPath -Description $displayName -IconLocation $paths.ProgramIcon
  }

  if ($StartMenuShortcut) {
    New-Shortcut -Path $paths.StartMenuShortcut -Target $paths.LoaderPath -Description $displayName -IconLocation $paths.ProgramIcon
  }
}
...
"@
  ...
  $aboutText.Text = @"
Ephemeral Session Browser ($Version), developed by shadow587.
...
function Main() {
  Write-Host "Ephemeral Session Browser Version $Version"
