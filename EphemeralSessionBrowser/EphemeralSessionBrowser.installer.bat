...
function Show-BaseProfileInfoMessage()
{
  try
  {
    $title = "Ephemeral Session Browser"
    $body = @"
The Ephemeral Session Browser has started in the base profile.
...
To access the base profile again, press SHIFT while starting the Ephemeral Session Browser.
"@
...
function Start-EphemeralBrowser()
{
    ...
    if (-not (Test-Path -Path $BaseProfilePath))
    {
